...
import tensorflow as tf, cpo_rl.utils.tf_util as U, numpy as np
...
class RunningMeanStd(object):
    ...
    def update(self, x):
        ...
        addvec = np.concatenate([x.sum(axis=0).ravel(), np.square(x).sum(axis=0).ravel(), np.array([len(x)],dtype='float632')])
        if MPI is not None:
