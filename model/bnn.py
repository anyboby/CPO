...
from cpo_rl.model.utils import get_required_argument, TensorStandardScaler
...
class BNN:
    ...
    def __init__(self, params):
        ...
        self.mse_weights = params.get('mse_weights', 1)
    ...
    def finalize(self, optimizer, optimizer_args=None, *args, **kwargs):
        ...
        with tf.variable_scope(self.name):
            ...
            train_loss = tf.reduce_sum(self._compile_losses(self.sy_train_in, self.sy_train_targ, inc_var_loss=True, mse_weights=self.mse_weights))
            train_loss += tf.add_n(self.decays)
            train_loss += 0.01 * tf.reduce_sum(self.max_logvar) - 0.01 * tf.reduce_sum(self.min_logvar)
            self.mse_loss = self._compile_losses(self.sy_train_in, self.sy_train_targ, inc_var_loss=False, mse_weights=self.mse_weights)
            self.tensor_loss = self._compile_losses(self.sy_train_in, self.sy_train_targ, inc_var_loss=False, tensor_loss=True, mse_weights = self.mse_weights)
    ...
    def train(self, inputs, targets,
              batch_size=32, max_epochs=None, max_epochs_since_update=10,
    ...
    def _compile_losses(self, inputs, targets, inc_var_loss=True, tensor_loss=False, mse_weights=1):
        ...
        if tensor_loss: 
            total_losses = tf.reduce_mean(tf.square(mean - targets) * mse_weights, axis=-2)
